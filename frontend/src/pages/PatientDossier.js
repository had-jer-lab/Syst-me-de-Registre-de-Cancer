...
const API = `http://${window.location.hostname}:8000/api`;  // ← IP automatique
        ...
        <div style={s.modalBody}>
          ...
          <div style={s.modalField}>
            ...
            <textarea style={{...s.modalInput, minHeight: 100, resize: 'vertical'}}
              placeholder="Observations, décisions…" value={form.compte_rendu}
...
export default function PatientDossier() {
  ...
  return (
    <div style={s.root}>
      ...
      {showConsultModal && (
        <AddConsultationModal patientId={id} onClose={() => setShowConsultModal(false)}
          onSaved={() => { setShowConsultModal(false); load(); showToast('✓ Consultation enregistrée'); }} />
      )}

      {/* ── QR Modal — يطلع في النص كيما الصورة ── */}
      {showQR && (
        <PatientQRSection
      ...
      <div style={s.header}>
        <div style={s.headerInner}>
          ...
          <div style={s.headerActions}>
            ...
            {/* ← زر QR في الهيدر — يفتح modal مباشرة */}
            ...
            <div style={s.colStack}>
              {dernierCancer ? (
                <SectionCard title="Dernier cancer enregistré" icon="🎗"
                  action={<button style={s.seeMoreBtn} onClick={() => setActiveTab('cancers')}>Voir tous →</button>}>
              ...
              {patient.consultations?.length > 0 ? (
                <SectionCard title="Dernière consultation" icon="📅"
                  action={<button style={s.seeMoreBtn} onClick={() => setActiveTab('consultations')}>Voir toutes →</button>}>
                  <div style={s.consultItem}>
                    <div style={s.consultDate}>{fmtDate(patient.consultations[0].consultation_date)}</div>
                    {patient.consultations[0].motif && <div style={s.consultMotif}>{patient.consultations[0].motif}</div>}
                    {patient.consultations[0].compte_rendu && <div style={s.consultCR}>{patient.consultations[0].compte_rendu}</div>}
                    {patient.consultations[0].next_visit_date && (
                      <div style={s.nextVisit}>📅 Prochain RDV : <strong>{fmtDate(patient.consultations[0].next_visit_date)}</strong></div>
                    )}
                ...
                <SectionCard title="Consultations" icon="📅">
                  ...
                  <button style={{ ...s.btnPrimary, marginTop: 14, width: '100%' }} onClick={() => setShowConsultModal(true)}>
        ...
        {activeTab === 'consultations' && (
          <div>
            <div style={s.consultHeader}>
              ...
              <button style={s.btnPrimary} onClick={() => setShowConsultModal(true)}>➕ Nouvelle consultation</button>
            </div>
            {patient.consultations?.length === 0 ? (
                ...
                {patient.consultations.map((c, i) => (
                  <div key={c.id} style={s.timelineItem}>
                    ...
                    <div style={s.timelineCard}>
                      ...
                      {c.compte_rendu && <div style={s.timelineCR}>{c.compte_rendu}</div>}
                      <div style={s.timelineFooter}>
                        {c.next_visit_date && <span style={s.nextVisit}>📅 Prochain : {fmtDate(c.next_visit_date)}</span>}
...
const s = {
  ...
  toast: { position: 'fixed', bottom: 24, right: 24, zIndex: 9999, background: '#1A2B4A', color: '#fff', padding: '13px 24px', borderRadius: 14, fontSize: 14, fontWeight: 700, boxShadow: '0 8px 24px rgba(0,0,0,0.18)' },
  header: { background: '#fff', borderBottom: '1px solid #E8EDF5', position: 'sticky', top: 0, zIndex: 100, boxShadow: '0 1px 4px rgba(0,0,0,0.06)' },
  ...
  backBtn: { padding: '7px 16px', borderRadius: 8, border: '1px solid #DDE4F3', background: '#F5F8FF', color: '#7A8BAD', fontSize: 13, fontWeight: 700, cursor: 'pointer', fontFamily: "'Nunito', sans-serif" },
  ...
  tnmTag: { padding: '4px 12px', borderRadius: 8, fontSize: 12, fontWeight: 800, background: '#EEF2FF', color: '#4A6CF7', fontFamily: "'Poppins', sans-serif", letterSpacing: '0.5px' },
  ...
  tab: { padding: '10px 18px', borderRadius: '10px 10px 0 0', border: '1px solid transparent', borderBottom: 'none', background: 'transparent', fontSize: 13, fontWeight: 700, color: '#7A8BAD', cursor: 'pointer', fontFamily: "'Nunito', sans-serif", transition: '0.15s', display: 'flex', alignItems: 'center', gap: 6 },
  ...
  sectionCard: { background: '#fff', borderRadius: 12, border: '1px solid #E8EDF5', overflow: 'hidden', boxShadow: '0 1px 4px rgba(0,0,0,0.04)' },
  ...
  seeMoreBtn: { fontSize: 12, fontWeight: 700, color: '#4A6CF7', background: 'none', border: 'none', cursor: 'pointer', fontFamily: "'Nunito', sans-serif" },
  ...
  cancerCard: { background: '#fff', borderRadius: 12, border: '1px solid #E8EDF5', marginBottom: 14, overflow: 'hidden', boxShadow: '0 1px 4px rgba(0,0,0,0.04)' },
  cancerCardToggle: { width: '100%', padding: '16px 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', border: 'none', background: '#FAFBFF', cursor: 'pointer', fontFamily: "'Nunito', sans-serif" },
  cancerCardLeft: { display: 'flex', alignItems: 'center', gap: 14 },
  cancerIndex: { width: 28, height: 28, borderRadius: 8, background: '#EEF2FF', color: '#4A6CF7', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, fontWeight: 800, flexShrink: 0 },
  ...
  examTableHeader: { display: 'grid', gridTemplateColumns: '1fr 1fr auto', background: '#F5F8FF', padding: '8px 14px', fontSize: 10.5, fontWeight: 800, color: '#7A8BAD', textTransform: 'uppercase', letterSpacing: '0.7px', gap: 12 },
  examRow: { display: 'grid', gridTemplateColumns: '1fr 1fr auto', padding: '9px 14px', fontSize: 13, gap: 12, borderTop: '1px solid #F0F4FF', alignItems: 'center' },
  ...
  metaItem: { padding: '6px 14px', borderRadius: 8, background: '#FFF5F5', border: '1px solid #FED7D7', display: 'flex', alignItems: 'center', gap: 8 },
  ...
  timelineDot: { position: 'absolute', left: -28, top: 16, width: 12, height: 12, borderRadius: '50%', background: '#4A6CF7', border: '2px solid #fff', boxShadow: '0 0 0 2px rgba(74,108,247,0.25)' },
  timelineLine: { position: 'absolute', left: -22, top: 28, width: 1, height: 'calc(100% + 20px)', background: '#DDE4F3' },
  timelineCard: { background: '#fff', borderRadius: 12, border: '1px solid #E8EDF5', padding: '14px 18px', boxShadow: '0 1px 4px rgba(0,0,0,0.04)' },
  ...
  modalClose: { width: 30, height: 30, borderRadius: 8, border: '1px solid #DDE4F3', background: '#F5F8FF', cursor: 'pointer', fontSize: 14, color: '#7A8BAD', display: 'flex', alignItems: 'center', justifyContent: 'center' },
  ...
  modalInput: { width: '100%', background: '#F5F8FF', border: '1.5px solid #DDE4F3', borderRadius: 10, padding: '10px 14px', fontSize: 13, fontFamily: "'Nunito', sans-serif", color: '#1A2B4A', outline: 'none', boxSizing: 'border-box' },
  ...
  btnGhost: { padding: '10px 18px', borderRadius: 30, border: '1.5px solid #DDE4F3', background: '#F5F8FF', color: '#7A8BAD', fontSize: 13, fontWeight: 700, cursor: 'pointer', fontFamily: "'Nunito', sans-serif" },
};