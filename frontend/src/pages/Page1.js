...
export default function Page1() {
  ...
  // Code 1 used `data.date_naissance`, code 2 used `data.dob`.
  // We keep BOTH keys synced so the rest of the app isn't broken.
  useEffect(() => {
    const dob = data.date_naissance || data.dob;
    if (dob) update({ age: calcAge(dob) });
  }, [data.date_naissance, data.dob]);

  const communesDispos = data.wilaya ? (WILAYAS_COMMUNES[data.wilaya] || []) : [];

  // Generic field setter (clears its error on change)
  const set = (key) => (e) => {
    update({ [key]: e.target.value });
    if (errors[key]) setErrors(p => ({ ...p, [key]: '' }));
  };

  // ── Voice + Scanner unified handler (from code 2) ──────────────────────────
  const handleVoiceFill = (fields) => {
    const mapped = {};

    // Identity – support both naming conventions
    if (fields.nom)         { mapped.last_name  = fields.nom;    mapped.nom    = fields.nom; }
    if (fields.prenom)      { mapped.first_name = fields.prenom; mapped.prenom = fields.prenom; }
    if (fields.last_name)   { mapped.last_name  = fields.last_name;  mapped.nom    = fields.last_name; }
    if (fields.first_name)  { mapped.first_name = fields.first_name; mapped.prenom = fields.first_name; }

    // DOB – unify keys
    if (fields.dob)         { mapped.dob = fields.dob; mapped.date_naissance = fields.dob; }
    if (fields.date_naissance) { mapped.date_naissance = fields.date_naissance; mapped.dob = fields.date_naissance; }

    // Direct mappings
    if (fields.sexe)        mapped.sexe              = fields.sexe;
    if (fields.famille)     mapped.situation_familiale = fields.famille;
    if (fields.couverture)  mapped.couverture_sociale  = fields.couverture;
    if (fields.tel)         { mapped.phone = fields.tel; mapped.tel = fields.tel; }
    if (fields.phone)       { mapped.phone = fields.phone; mapped.tel = fields.phone; }
    if (fields.email)       mapped.email             = fields.email;
    if (fields.wilaya)      { mapped.wilaya = fields.wilaya; mapped.commune = ''; }
    if (fields.commune)     mapped.commune            = fields.commune;
    if (fields.adresse)     mapped.adresse            = fields.adresse;
    if (fields.profession)  mapped.profession         = fields.profession;

    // NIN – ID card scanner uses `national_id`, voice uses `nin`
    if (fields.national_id) { mapped.national_id = fields.national_id; mapped.nin = fields.national_id; }
    if (fields.nin)         { mapped.nin = fields.nin; mapped.national_id = fields.nin; }
  ...
  const handleNext = () => {
    ...
    if (!data.date_naissance && !data.dob)
                                     errs.date_naissance = 'La date de naissance est obligatoire';
  ...
  // Resolved values (handle both key conventions)
  const dobValue = data.date_naissance || data.dob || '';
      ...
      <div className="grid-2">
        ...
        <div className="col-stack">
          ...
          <SC label="Identité du patient">
            <div className="field-row c2">
              <div className="fg">
                ...
                <MicField onResult={(t) => { update({ last_name: t, nom: t }); if (errors.last_name) setErrors(p => ({ ...p, last_name: '' })); }}>
                  <input
              ...
              <div className="fg">
                ...
                <MicField onResult={(t) => { update({ first_name: t, prenom: t }); if (errors.first_name) setErrors(p => ({ ...p, first_name: '' })); }}>
                  <input
          ...
          <SC label="Date de naissance">
            <div className="fg">
              ...
              <input
                ...
                value={dobValue}
                onChange={(e) => {
                  update({ date_naissance: e.target.value, dob: e.target.value });
                  if (errors.date_naissance) setErrors(p => ({ ...p, date_naissance: '' }));
                }}
          ...
          <SC label="Identifiant national">
            <div className="fg">
              ...
              <MicField onResult={(t) => { const v = t.replace(/\D/g,'').slice(0,18); update({ national_id: v, nin: v }); if (errors.national_id) setErrors(p => ({ ...p, national_id: '' })); }}>
                <div className="fi-wrap">
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      update({ national_id: val, nin: val });
        ...
        <div className="col-stack">
          ...
          <SC label="Adresse">
            ...
            {data.wilaya && communesDispos.length > 0 && (
