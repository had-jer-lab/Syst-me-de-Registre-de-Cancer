...
export default function PatientQRCode({ patient }) {
  ...
  const publicUrl = `${baseOrigin}/#/patient-public/${patient.id}?token=${btoa(patient.numero_dossier || String(patient.id))}`;
        ...
        <div style={s.backdrop} onClick={e => e.target === e.currentTarget && setShowModal(false)}>
          <div style={s.modal}>
            ...
            <div style={s.modalBody}>
              ...
              <div style={s.infoBox}>
                ...
                <div style={s.infoRow}>
                  ...
                  <span style={{ ...s.infoValue, color:'#4A6CF7' }}>{patient.numero_dossier}</span>
                </div>
                <div style={s.infoRow}>
                  <span style={s.infoLabel}>Date naissance</span>
                  <span style={s.infoValue}>
                    {patient.date_naissance ? patient.date_naissance.split('-').reverse().join('/') : '—'}
                  </span>
                </div>
              </div>
              ...
              {/* Info téléphone — uniquement en dev */}
              {(window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1') && (
                <div style={s.devBox}>
                  <span style={{ fontSize:14, flexShrink:0 }}>📱</span>
                  <span>
                    Pour scanner depuis un téléphone, assurez-vous qu'il est connecté
                    au même réseau Wi-Fi. Adresse utilisée : <strong>{baseOrigin}</strong>
                  </span>
                </div>
              )}
            ...
            <div style={s.modalFoot}>
              ...
              <button style={s.btnPrint} onClick={handlePrint}>🖨 Imprimer le QR Code</button>
            </div>
...
export function QRMiniCard({ patient }) {
  const publicUrl = `${getPublicUrl()}/#/patient-public/${patient?.id}?token=${btoa(patient?.numero_dossier || String(patient?.id || '0'))}`;
  return (
    <div style={{ display:'flex', flexDirection:'column', alignItems:'center', gap:4 }}>
      <img src={getQRUrl(publicUrl, 80)} alt="QR" style={{ width:60, height:60, borderRadius:8, border:'1px solid #E8EDF5' }} />
      <div style={{ fontSize:10, color:'#7A8BAD', fontWeight:700 }}>QR Dossier</div>
    </div>
  );
}
...
const s = {
  ...
  backdrop: {
    ...
    background:'rgba(10,20,50,0.55)', backdropFilter:'blur(8px)',
  ...
  modalHead: {
    ...
    padding:'18px 22px', borderBottom:'1px solid #F0F4FF', flexShrink:0,
  },
  modalHeadIcon: {
  ...
  qrCard: {
    background:'#fff', borderRadius:16, border:'2px solid #E8EDF5',
    padding:'20px 28px', textAlign:'center',
    boxShadow:'0 4px 20px rgba(74,108,247,0.08)', width:260,
  },
  qrLogo:    { fontFamily:"'Poppins', sans-serif", fontWeight:800, fontSize:15, color:'#4A6CF7', marginBottom:14 },
  qrImg:     { width:200, height:200, display:'block', margin:'0 auto', borderRadius:8, border:'1px solid #F0F4FF' },
  qrName:    { fontFamily:"'Poppins', sans-serif", fontWeight:700, fontSize:14, color:'#1A2B4A', marginTop:14, marginBottom:4 },
  qrDossier: { fontSize:12, color:'#7A8BAD', fontWeight:700, marginBottom:10 },
  qrHint:    { fontSize:11, color:'#7A8BAD', fontWeight:600, padding:'6px 12px', background:'#F5F8FF', borderRadius:20, display:'inline-block' },

  infoBox: {
    background:'#F8FAFC', borderRadius:12,
    border:'1px solid #E8EDF5', marginBottom:14, overflow:'hidden',
  },
  infoRow: {
    display:'flex', alignItems:'center', justifyContent:'space-between',
  ...
  devBox: {
    display:'flex', gap:8, padding:'10px 14px', marginBottom:12,
    background:'#dbeafe', border:'1px solid rgba(59,130,246,0.25)',
    borderRadius:10, fontSize:12, color:'#1e40af', fontWeight:600, lineHeight:1.5,
  },
  warning: {
    display:'flex', gap:8, padding:'10px 14px',
    background:'#fef3c7', border:'1px solid rgba(217,119,6,0.25)',
    borderRadius:10, fontSize:12, color:'#92400e', fontWeight:600, lineHeight:1.5,
  },
