...
from . import views
from .views import check_pending_meetings, set_vote_proposal, close_vote, add_vote_proposal
...
urlpatterns = [
    ...
    path('<int:rcp_id>/add-proposal/',       add_vote_proposal,            name='rcp-add-proposal'),
    path('<int:rcp_id>/validate/',           views.validate_decision,      name='rcp-validate'),

    # ── 📹 Vidéo-conférence ──────────────────────────────────────
    path('<int:rcp_id>/start-video/',        views.start_video,            name='rcp-start-video'),
    path('<int:rcp_id>/end-video/',          views.end_video,              name='rcp-end-video'),
